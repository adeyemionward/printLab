<?php
    ...
    use App\Models\MemberCooperativeCurrentMoney;
    ...
    class CooperativeService
    {
        public function postContributionPayment($data){
            ...
            try{
                ...
                $payment = new CooperativeContributionPayment();
                ...
                $payment->save();
                $receiverCustomer = MemberCooperativeCurrentMoney::where('member_id', request('member_id'))->first();

                    // If the record does not exist, create a new one
                    if (!$receiverCustomer) {
                        $receiverCustomer = MemberCooperativeCurrentMoney::firstOrCreate(
            ...
            return redirect(route('dashboard.admin.cooperative.contributions.payments.index'))->with('flash_success','payment saved successfully');
        }
        ...
        public function postContributionPayout($data){
            ...
            try{
                ...
                $payment = new CooperativeContributionPayout();
                $payment->member_id        = request('member_id');
                $payment->amount           = request('amount');
                $payment->date             = request('date');
                $payment->created_by       = $user_logged->id;
            ...
            return redirect(route('dashboard.admin.cooperative.contributions.payouts.index'))->with('flash_success','payout saved successfully');
        ...
        public function postLoanRepayment($data){
            ...
            try{
                ...
                $payment = new CooperativeLoanRepayment();
                $payment->member_id        = request('member_id');
                ...
                $receiverCustomer = CooperativeLoanPayout::where('id',request('loan_payout_id'))->first();
                ...
                if(!is_null(request('use_savings'))){
                    //deduct money from cooperative savings when loan is paid with saving using the check box
                    $receiverCustomer = MemberCooperativeCurrentMoney::where('member_id', request('member_id'))->first();
                    ...
                    //store in the CooperativeContributionPayout for proper docs
                    $payment = new CooperativeContributionPayout();
                    $payment->member_id        = request('member_id');
                    $payment->amount           = request('amount');
                    $payment->date             = request('date');
            ...
            return redirect(route('dashboard.admin.cooperative.loans.repayments.index'))->with('flash_success','payout saved successfully');
        ...
        public function postLoanPayout($data){
            ...
            try{
                ...
                $payment = new CooperativeLoanPayout();
                $payment->member_id        = request('member_id');
            ...
            return redirect(route('dashboard.admin.cooperative.loans.payouts.index'))->with('flash_success','loan payout saved successfully');
